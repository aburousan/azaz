...
The tree walk and the multipole recursion are listed and explained [in part 3](/Pages/Physics/papers/hernquist1987/03_treecode/), and the integrator [in part 2](/Pages/Physics/papers/hernquist1987/02_leapfrog/), so I will not repeat them. Below are the parts that have not appeared yet.
...
If this passes, the tree structure and the walk are both right, leaving only the multipole terms to check.

**The quadrupole recursion must be right.** Build the tree, then compare the **root** node's $\mathbf{Q}$ against a brute-force sum over all $N$ particles. The root never touches a particle directly, since it only adds up its eight children, so an error anywhere in the recursion shows up here:
...
**Poisson's equation.** The density and potential pair, checked numerically as well as symbolically:
...
Two of these tests failed the first time, and both failures were **real physics** rather than bugs. The virial ratio came out at 0.94 instead of 1, which was the softening, now explained in part 1 rather than papered over. A truncated Plummer sphere is also not exactly virialised, which I confirmed by removing the cutoff and watching the ratio walk back to 1.
...
| What | Result |
| --- | --- |
| Legendre expansion of the inverse distance, orders 0 to 4 | exact |
