...
(function() {
  ...
  brd.create('functiongraph', [f1, -5, 9],
    { strokeColor: '#0047AB', strokeWidth: 2.5 });
  brd.create('functiongraph', [f2, -1, 9],
    { strokeColor: '#178F3E', strokeWidth: 2.5 });

  // Slider for HEIGHT
  const s = brd.create('slider', [[-5,18],[8,18],[0,4,16]],
    { name:'y₀', snapWidth:0.1 });

  // Branch sign from dropdown
  ...
  // Helpers for the two contact x-positions at the current height
  const xA = () => branch * Math.sqrt(Math.max(s.Value(), 0));
  const xB = () => 4 + branch * Math.sqrt(Math.max(s.Value(), 0));

  // --- Static "fan" of parallel tangent pairs (drawn ONCE, no runtime churn) ---
  // This shows the parallel-at-equal-heights idea persistently and cheaply.
  // We only rebuild it when the branch flips, not every frame.
  let fan = [];
  function drawFan() {
    fan.forEach(o => brd.removeObject(o));
    fan = [];
    for (let h = 1.5; h <= 16; h += 2) {
      const a = branch * Math.sqrt(h), m1 = 2 * a, c1 = h - m1 * a;
      const b = 4 + branch * Math.sqrt(h), m2 = 2 * (b - 4), c2 = h - m2 * b;
      fan.push(brd.create('line', [[-6, m1*-6 + c1], [10, m1*10 + c1]],
        { strokeColor:'#1CA9C9', strokeWidth:1, strokeOpacity:0.16, fixed:true, highlight:false }));
      fan.push(brd.create('line', [[-6, m2*-6 + c2], [10, m2*10 + c2]],
        { strokeColor:'#178F3E', strokeWidth:1, strokeOpacity:0.16, fixed:true, highlight:false }));
    }
  }
  drawFan();

  // Dashed equal-height connector — makes "same height" obvious
  const P1 = brd.create('point', [xA, ()=>s.Value()],
    { name:'P₁', color:'#D13F32', size:4, fixed:true, label:{offset:[8,8]} });
  const P2 = brd.create('point', [xB, ()=>s.Value()],
    { name:'P₂', color:'#E6B800', size:4, fixed:true, label:{offset:[8,8]} });
  brd.create('segment', [P1, P2],
    { strokeColor:'#888', strokeWidth:1, dash:2, fixed:true });

  // Bold live tangents at the two points (only 2 dynamic curves on the board)
  brd.create('line', [
    ()=>[xA(), s.Value()],
    ()=>[xA() + 1, s.Value() + df1(xA())]
  ], { strokeColor:'#1CA9C9', strokeWidth:3, fixed:true });
  brd.create('line', [
    ()=>[xB(), s.Value()],
    ()=>[xB() + 1, s.Value() + df2(xB())]
  ], { strokeColor:'#178F3E', strokeWidth:3, fixed:true });

  // Rebuild the static fan when the branch changes
  document.getElementById("branchSelect").addEventListener("change", () => { drawFan(); brd.update(); });

  // Live slope readout — the heart of the lesson: both slopes are EQUAL
  brd.create('text', [-5.5, 16, 'Blue: f₁(x)=x²'], {fontSize:15, color:'#0047AB', fixed:true});
  brd.create('text', [-5.5, 15, 'Green: f₂(x)=(x−4)²'], {fontSize:15, color:'#178F3E', fixed:true});
  brd.create('text', [-5.5, 14, 'Tangents stay parallel at equal heights'], {fontSize:13, color:'#333', fixed:true});
  brd.create('text', [-5.5, 12.7,
    () => 'slope on both branches = ' + df1(xA()).toFixed(2)
  ], {fontSize:15, color:'#9C27B0', fixed:true, cssStyle:'font-weight:bold;'});

  // No autoplay — drag the y₀ slider yourself (or let the cat grab it).
})();
</script>
~~~
